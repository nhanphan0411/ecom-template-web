...
export default function AdminPage() {
  ...
  useEffect(() => {
    fetch("/api/admin/collections").then(res => res.json()).then(setCollections);
  }, []);

  useEffect(() => {
    setProducts([]); setProductSlug("");
    setValue1Options([]); setValue1("");
    setValue2Options([]); setValue2("");
    setStaged([]); setOriginalIds([]); setDirty(false);

    if (!collectionSlug) return;
    fetch(`/api/admin/products?collection=${collectionSlug}`).then(res => res.json()).then(setProducts);
  }, [collectionSlug]);
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/value1?product=${productSlug}`).then(res => res.json()).then(setValue1Options);
  }, [productSlug]);
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/value2?product=${productSlug}&value1=${value1}`).then(res => res.json()).then(setValue2Options);
  }, [productSlug, value1]);
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/images?${params.toString()}`)
      .then(res => res.json())
      .then((imgs: any[]) => {
        setStaged(imgs.map(img => ({ kind: "existing", id: img.id, url: img.url })));
  ...
  async function handleSync() {
    ...
    const stagedExistingIds = staged
      .filter((s): s is Extract<StagedImage, { kind: "existing" }> => s.kind === "existing")
      .map(s => s.id);
    ...
    const order = staged.map(s =>
      s.kind === "existing"
        ...
        : { type: "new", fileIndex: newFiles.findIndex(f => f.tempId === s.tempId) }
    );
    ...
    for (const f of newFiles) formData.append("file", f.file);

    const res = await fetch("/api/admin/images/sync", { method: "POST", body: formData });
    const images = await res.json();

    setStaged(images.map((img: any) => ({ kind: "existing", id: img.id, url: img.url })));
      ...
      <div style={{ display: "flex", gap: 16, marginTop: 24 }}>
        <select value={collectionSlug} onChange={e => setCollectionSlug(e.target.value)}>
          ...
          {collections.map((c: any) => (
            <option key={c.collection_slug} value={c.collection_slug}>{c.collection_name}</option>
          ))}
        </select>

        <select value={productSlug} onChange={e => setProductSlug(e.target.value)} disabled={!collectionSlug}>
          <option value="">Select product</option>
          {products.map((p: any) => (
            <option key={p.product_slug} value={p.product_slug}>{p.product_name}</option>
          ))}
        </select>

        <select value={value1} onChange={e => setValue1(e.target.value)} disabled={!productSlug}>
          <option value="">Select variant 1</option>
          {value1Options.map((v: any) => (
            <option key={v.value1} value={v.value1}>{v.value1}</option>
          ))}
        </select>

        {needsValue2 && (
          <select value={value2} onChange={e => setValue2(e.target.value)} disabled={!value1}>
            <option value="">Select variant 2</option>
            {value2Options.map((v: any) => (
              <option key={v.value2} value={v.value2}>{v.value2}</option>
            ))}
          </select>
        )}
      </div>
      ...
      {readyToShowImages && (
        <div style={{ marginTop: 24 }}>
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            ...
            {dirty && !syncing && <span style={{ color: "orange" }}>Unsaved changes</span>}
          </div>
              ...
              >
                <img
                  ...
                  style={{
                    ...
                    border: img.kind === "new" ? "2px dashed orange" : "none",
                  }}
                />
                <button onClick={() => handleRemove(index)} style={{ position: "absolute", top: 4, right: 4 }}>✕</button>
              </div>
            ))}
          </div>
        </div>
